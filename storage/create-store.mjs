...
export function createStore({ kind, root, repository, token, maximumArtifactBytes, signal, fetchImplementation }) {
  if (kind === "directory") {
  ...
  if (kind === "github") {
    return new GitHubReleaseStore({ repository, token, maximumArtifactBytes, signal, fetchImplementation });
  }
