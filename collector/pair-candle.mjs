...
export function validatePairCoverage(value, label = "coverage") {
  ...
  if (fromBlock > untilBlock || fromTime >= untilTime) throw new Error(`${label} is inverted.`);
  return value;
}
...
export function validateCandleSwapSpan(first, last, tradeCount) {
  ...
  if (count === 1n && order !== 0 || count > 1n && order === 0) {
...
export function validatePairCandleSequence(value, candleContext = {}) {
  ...
  for (const candle of value) {
    ...
    if (previous !== undefined) {
      if (candle.intervalStart <= previous.intervalStart) throw new Error("Pair candles are duplicated or unordered.");
